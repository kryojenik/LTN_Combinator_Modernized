...
for _, surface in pairs(game.surfaces) do
  ...
  for _, entity in pairs(entities) do
    ...
    local sig = ctl.get_section(1).get_slot(config.ltn_signals["ltn-depot"].slot)
    if sig.signal and sig.signal.name == "ltn-depot" and sig.count > 0 then
      ...
      for signal_name, details in pairs(config.ltn_signals) do
        if details.group == "provider" or details.group == "requester" then
          ctl.get_section(1).clear_slot(details.slot)
        end
      end
      goto continue
    end
    ...
    for _, service in ipairs{ "provider", "requester" } do
      ...
      local sig = ctl.get_section(1).get_slot(config.ltn_signals[name].slot)
      ...
      if sig.signal and sig.signal.name == name then
        if sig.count >= config.old_high_threshold then
          cd[service] = false
          ctl.get_section(1).set_slot(config.ltn_signals[name].slot, {
      ...
      sig = ctl.get_section(1).get_slot(config.ltn_signals[name].slot)
      ...
      if sig.signal and sig.signal.name == name and sig.count ~= 0 then
        ...
        if not cd[service] then
          ctl.get_section(1).clear_slot(config.ltn_signals[name].slot)
        end
      end
    end
